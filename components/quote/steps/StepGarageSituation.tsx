...
const OPTIONS: Array<{
  ...
  { value: 'no-garage', label: 'No garage — outdoor install' },
]

export function StepGarageSituation({
  garageSituation,
  onFieldUpdate,
  onNext,
  onBack,
  ...
  const canProceed = garageSituation !== ''
  ...
  return (
    <div className="space-y-6 px-4 py-8 sm:px-0 sm:py-12">
      <fieldset>
        <legend className="sr-only">What is your garage situation?</legend>
        <h2
          ref={headingRef}
          id="step-heading"
          tabIndex={-1}
          ...
          {OPTIONS.map(({ value, label }) => {
            ...
            return (
              <button
                ...
                className={cn(
                  'flex min-h-16 w-full items-center justify-between gap-2 rounded-xl border-2 px-4 py-3 text-left text-base font-medium transition-colors',
                  selected
